...
void ExtensionAdapter::EntryPointsSetExtraJSEntryPoints(XW_Extension xw_extension,
  ...
  // Clear the existing entry_points when calling the RegisterExtensionsByMetadata function
  extension->entry_points_.clear();
...
void ExtensionAdapter::DataRegisterAsync(
    XW_Extension xw_extension,
...
void ExtensionAdapter::DataSetSyncReply(
    XW_Instance xw_instance,
    const char* reply, uint8_t* buffer, size_t len) {
  ExtensionInstance* instance = GetExtensionInstance(xw_instance);
  CHECK(instance, xw_instance);
  instance->SyncReplyToJS(reply, buffer, len);
}

void ExtensionAdapter::DataPostData(
    XW_Instance xw_instance,
    const char* message, uint8_t* buffer, size_t len) {
  ExtensionInstance* instance = GetExtensionInstance(xw_instance);
  CHECK(instance, xw_instance);
  instance->PostMessageToJS(message, buffer, len);
}
